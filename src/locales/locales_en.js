export const en = {
  add_a_circle: 'Add a circle',
  add_a_free_hand_shape: 'Add a free hand shape',
  add_a_line: 'Add a line',
  add_a_rectangle: 'Add a rectangle',
  add_an_arrow_shape: 'Add an arrow shape',
  add_an_ellipse: 'Add an ellipse',
  add_image_from_url: 'Add image from URL',
  anonymous: 'Anonymous',
  applyMerge: 'Apply',
  arrow: 'Arrow',
  border_color: 'Border color',
  cancel: 'Cancel',
  circle: 'Circle',
  click_on_canvas_to_write: ' Click on canvas to write text',
  click_to_delete_shape: 'Click on shape to delete it',
  confirmTemplateSelectionContent: 'Your note already has content. Review the template below, then edit your note to merge in what you need.',
  confirmTemplateSelectionTitle: 'Merge template with your note',
  container_size: 'Container Size',
  create_annotation: 'Create annotation',
  createdByOn: 'Created by {{creator}}, {{creationDate}}',
  currentNoteContent: 'Your note',
  debugAnnotation: 'Debug annotation in console',
  delete: 'Delete',
  delete_all: 'Delete all',
  deleteAnnotation: 'Delete',
  detected_media_type: 'We detect {{mediaType}} media type.',
  document: 'Document',
  drawings_and_text_in_overlay: 'Drawings and text in overlay',
  edit: 'Edit',
  edit_annotation: 'Edit annotation',
  edit_iiif_json_code: 'Edit directly the IIIF json code',
  ellipse: 'Ellipse',
  expert_mode: 'Expert mode',
  export_annotation: 'Export Annotations',
  export_annotation_for: 'Export annotation for {{label}}',
  export_local_annotation: 'Export local annotations for visible items',
  fill_color: 'Fill color',
  freehand: 'Freehand',
  image: 'Image',
  image_displayed: 'Image Displayed',
  image_in_overlay_with_note: 'Image in overlay with a note',
  image_true_size: 'Image True Size',
  imageUrl: 'Image URL',
  journey: 'Journey',
  journey_description: 'An ordered sequence of POIs, with a title and optional description',
  journey_description_placeholder: 'Describe this journey…',
  journey_description_section: 'Description',
  journey_hint: 'Use the left annotation list to associate POIs to journey and order them',
  journey_language: 'Language',
  journey_media_ar: 'Media (Arabic)',
  journey_media_en: 'Media (English)',
  journey_media_keep_same: 'Keep same media for both languages',
  journey_title: 'Title',
  journey_title_required: 'Enter a title in each language.',
  lastEditedByOn: 'Last edit by {{lastEditor}}, {{lastSavedDate}}',
  line: 'Line',
  line_weight: 'Line weight',
  loading_failed: 'Loading failed',
  maev_github_link: 'https://github.com/SCENE-CE/mirador-annotation-editor-video',
  manifest_link: 'Manifest annotation',
  manifest_link_with_note: 'Manifest with note',
  manifest_url: 'Manifest URL',
  map_relation_no_results: 'No matching maps',
  map_relation_search_prompt: 'Start typing to search maps',
  media_not_supported: 'Media not supported',
  media_not_supported_text: 'The media you try to annotate is not supported by this instance at the moment',
  metadata: 'Metadata',
  move_to_journey: 'Move to journey',
  nested_map: 'Nested map',
  nested_map_description: 'Point of interest that opens another map when clicked',
  nested_map_linked_map: 'Linked map',
  nested_map_linked_map_required: 'Choose a map for this point to link to.',
  new_annotation: 'New annotation',
  no_annotation: 'No annotations stored yet.',
  note: 'Note',
  object_list: 'Object list',
  onlyOneImageAllowed: 'Only one image is allowed',
  openNestedMap: 'Open map',
  overlay: 'Overlay',
  poi: 'POI',
  poi_click_to_place: 'Click on the image to place the POI marker. Click again to move it.',
  poi_description: 'Point of interest with a target, a description and optional links',
  poi_description_placeholder: 'Describe this point of interest…',
  poi_description_section: 'Description',
  poi_language: "Language",
  poi_latitude: 'Latitude',
  poi_latitude_invalid: 'Latitude must be between -90 and 90.',
  poi_longitude: 'Longitude',
  poi_longitude_invalid: 'Longitude must be between -180 and 180.',
  poi_media_ar: 'Media (Arabic)',
  poi_media_en: 'Media (English)',
  poi_media_keep_same: 'Keep same media for both languages',
  poi_media_no_results: 'No matching results',
  poi_media_search_prompt: 'Start typing to search',
  poi_media_source_iiif_image: 'IIIF image',
  poi_media_source_media_item: 'Media item',
  poi_media_source_upload: 'Media library',
  poi_mode: 'Map interaction',
  poi_mode_move: 'Move around the map',
  poi_mode_target: 'Place the point',
  poi_move_hint: 'Drag to move around the map. Switch to the target tool, top right of the map, to place or move the point.',
  pois_mode_target: 'Move the points',
  poi_target_must_be_point: 'Click on the image to set this POI\'s location.',
  poi_title: 'Title',
  poi_title_required: 'Enter a title in each language.',
  polygon: 'Polygon',
  previewAnnotation: 'Preview',
  pressEnterOrClickFirstPointToClose: 'Click first point or press "Shift" to close polygon, "Tab" to leave open',
  pressEnterToAddTag: 'Add your tag and press "Enter"',
  pressEscapeToFinish: 'Press "Esc" to finish the line',
  rectangle: 'Rectangle',
  refresh_journey_path: 'Refresh path',
  remove_from_journey: 'Remove from journey',
  replace: 'Replace',
  save: 'Save',
  saving: 'Saving…',
  scale: 'Scale',
  select_cursor: 'Select Cursor',
  selected_object: 'Selected object',
  selectedTemplateContent: 'Selected template',
  shape: 'Shape',
  shape_selection: 'Draw shapes',
  shapes: 'Shapes',
  spatialTarget: 'Shapes',
  style: 'Style',
  style_selection: 'Style selection',
  switch_view: 'Switch to single view',
  switch_view_content: 'Annotations can only be edited in single canvas view type.\n'
    + 'Switch view type to single view now?',
  switch_view_h2: 'Switch view type to single view?',
  tag: 'Tag',
  tag_with_target: 'Tag with target',
  tags: 'Tags',
  tagsPlaceholder: 'Add tags ...',
  target: 'Target',
  text: 'Text',
  textual_note_with_target: 'Textual note with target',
  tool_selection: 'Tool Selection',
  unsupported_media_message: 'Your current canvas media type is not supported by the annotation editor.',
  useTemplate: 'Use a template',
  video_annotation_instruction: 'If you want to annotate video media, you must install MAEV to create and edit annotations on video:',
  your_tag_here: 'Your tag here:',
  zoom: 'Zoom',
};
